...
Serves the site from the src/ directory.
"""
...
class AutoRefreshHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, directory=str(SRC_ROOT), **kwargs)
    ...
    def do_GET(self):
        ...
        if event_match:
            ...
            if (
                ...
                and '.' not in segment
                and not (SRC_ROOT / 'calendar' / segment).exists()
            ):
...
def watch_files():
    ...
    while True:
        try:
            ...
            # Check all watched files under src/
                        ...
                        try:
                            ...
                            if file_str in file_mtimes:
                                if current_mtime > file_mtimes[file_str]:
                                    print(f"File changed: {file_path.relative_to(SRC_ROOT)}")
                                    changed = True
...
def main():
    ...
    if not SRC_ROOT.is_dir():
        raise SystemExit(f"Site directory not found: {SRC_ROOT}")
